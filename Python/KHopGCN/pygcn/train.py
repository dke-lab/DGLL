...
from pygcn.utils import load_data,load_Khopdata, accuracy
...
def main_GCN():
    ...
    # for ind in index[2]:
    #     if ind[0] == 3192:
    #         adj, features, labels = load_Khopdata(3192, ind[1])
    ...
    for x in range(numOfNodes):
        ...
        for ind in index[0]:
            if ind[0] == int(nodeID):
                adj, features, labels, neighborNumber = load_Khopdata(nodeID, ind[1])
                ...
                # Train model
                t_total = time.time()
                for epoch in range(neighborNumber):
                    train(epoch)
